...
import { COLORS, cardRoot, formatAmount, trendLabel, timeAgo } from "./theme";
  ...
  return (
    <FlexWidget clickAction="OPEN_URI" clickActionData={{ uri: DEEP_LINK }} style={cardRoot}>
      ...
      <TextWidget
        ...
        maxLines={1}
        style={{ fontSize: 32, color: COLORS.white, fontWeight: "700", marginTop: 6 }}
      />
      <TextWidget
        text={spendLine}
        maxLines={1}
        style={{ fontSize: 12, color: COLORS.muted, fontWeight: "500", marginTop: 6 }}
      />
      {updated ? (
        <TextWidget
          text={`Updated ${updated}`}
          style={{ fontSize: 10, color: COLORS.muted, fontWeight: "400", marginTop: 4 }}
        />
      ) : null}
    </FlexWidget>
  );
