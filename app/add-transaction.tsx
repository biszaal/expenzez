...
export default function AddTransactionScreen() {
  ...
  const handleSaveTransaction = async () => {
    ...
    if (!description.trim()) {
      Alert.alert(
        "Missing Description",
        `Please enter a description for this ${transactionType}.`
      );
      return;
    }
    ...
    try {
      ...
      if (result) {
        try {
          ...
          if (transactionType === "expense") {
            ...
            const { shouldAwardWeekly, newStreak } =
              await StreakService.recordDailyActivity();

            if (shouldAwardWeekly) {
              console.log(
                `🔥 [AddTransaction] 7-day streak achieved! Current streak: ${newStreak} days`
              );
      ...
      if (result) {
        // Save transaction data to AsyncStorage for immediate display on home screen
        ...
        try {
          ...
          console.log(
            "💰 [AddTransaction] Transaction saved to AsyncStorage for immediate display:",
            newTransaction
          );
        } catch (error) {
          console.error(
