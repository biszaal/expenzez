...
import BillTrackingAlgorithm, {
  DetectedBill,
} from "../../services/billTrackingAlgorithm";
...
export default function BillsScreen() {
    ...
    async (showRefresh = false) => {
      try {
        ...
        if (showRefresh) {
          ...
          console.log(
            "[Bills] Manual refresh - invalidating cache and running fresh detection..."
          );
          ...
          console.log(
            "[Bills] Loading bills with improved merchant filtering..."
          );
        ...
        try {
          ...
          const mergedBills =
            await BillPreferencesAPI.mergeBillsWithPreferences(
              detectedBills,
              userPreferences
            );
          console.log(
            `[Bills] Successfully merged with ${userPreferences.length} user preferences`
          );
          finalBills = mergedBills;
          setBills(mergedBills);
        } catch (error) {
          console.log(
            `[Bills] Bill preferences API not available yet, using detected bills only:`,
        ...
        if (showRefresh) {
          ...
          console.log(
            `[Bills] Loaded ${finalBills.length} bills from cache/detection`
          );
        }
      } catch (error) {
  ...
  const handleExcludeBill = useCallback(
    async (
      reason: "not_recurring" | "no_longer_active" | "incorrect_detection"
    ) => {
      ...
      try {
        ...
        setBills((prevBills) =>
          prevBills.filter((bill) => bill.id !== billToExclude.id)
        );
        ...
        showSuccess(
          `✓ Removed "${billToExclude.merchant}" - marked as ${reasonText}`
        );
  ...
  const filteredBills = useMemo(() => {
    ...
    return bills.filter(
      (bill) =>
        bill.category === selectedCategory && bill.status !== "cancelled"
    );
  ...
  const getAnalysisForBill = (bill: DetectedBill) => {
    if (!bill || !bill.transactions || bill.transactions.length === 0)
      return null;

    const transactions = bill.transactions.map((t) => ({
      ...t,
    ...
    const monthlyData = sortedEntries.map(([month, amount], index) => {
      ...
      const displayMonth =
        isJanuary || isYearChange || isFirstMonth
      ...
      return {
        ...
        count: transactions.filter(
          (t) => dayjs(t.date).format("YYYY-MM") === month
        ).length,
      };
    });
    ...
    const percentageIncrease =
      firstAmount > 0 ? (totalIncrease / firstAmount) * 100 : 0;
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: () =>
            isInactive ? reactivateBill(bill) : markBillInactive(bill),
          style: "default",
        },
        {
  ...
  const showCategoryEditor = (bill: DetectedBill) => {
    Alert.alert("Change Category", `Select a new category for ${bill.name}:`, [
  ...
  const updateBillCategory = async (
    bill: DetectedBill,
    newCategory: string
  ) => {
    try {
      try {
        ...
        console.warn(
          "Backend API not available, using local storage only:",
          apiError
        );
  ...
  const removeBill = async (bill: DetectedBill) => {
    try {
      ...
      const preference = await BillPreferencesAPI.createPreferenceFromBill(
        bill,
        {
          isIgnored: true,
  ...
  const showAddBillOptions = () => {
    Alert.alert("Add New Bill", "How would you like to add a new bill?", [
      {
      ...
      {selectedBill && (
        <BillDetailsModal
          ...
          onViewTransactions={() => {
            setModalVisible(false);
            router.push(
              `/transactions?merchant=${encodeURIComponent(selectedBill.merchant)}`
            );
          }}
          onManageBill={() => {
            setModalVisible(false);
            ...
            <View style={excludeModalStyles.options}>
              ...
              >
                ...
                <View style={excludeModalStyles.optionText}>
                  <Text
                    style={[
                      excludeModalStyles.optionTitle,
                      { color: colors.text.primary },
                    ]}
                  >
                  ...
                  <Text
                    style={[
                      excludeModalStyles.optionDescription,
                      { color: colors.text.secondary },
                    ]}
                  >
