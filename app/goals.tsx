...
import { goalsAPI, GoalsResponse, FinancialGoal, CreateGoalRequest } from '../services/api/goalsAPI';
import {
  GoalsOverview,
  GoalCard,
  CreateGoalForm
...
export default function GoalsScreen() {
  ...
  // Load goals data
  const loadGoalsData = useCallback(async (isRefresh = false) => {
  ...
  const handleQuickAction = (goalId: string, action: 'contribute' | 'edit' | 'pause') => {
          ...
          <View style={styles.section}>
            ...
            <View style={styles.recommendationsContainer}>
              {goalsData.recommendations.map((recommendation, index) => {
                const typeColor = goalsAPI.getGoalTypeColor(recommendation.type);
                const typeIcon = goalsAPI.getGoalTypeIcon(recommendation.type);
                const priorityColor = goalsAPI.getPriorityColor(recommendation.priority);

                return (
                  <View key={index} style={[styles.recommendationCard, { borderLeftColor: typeColor }]}>
                    <View style={styles.recommendationHeader}>
                      <View style={[styles.recommendationIcon, { backgroundColor: typeColor + '20' }]}>
                      ...
                      <View style={styles.recommendationInfo}>
                        ...
                        <View style={[styles.recommendationPriority, { backgroundColor: priorityColor + '20' }]}>
                          <Text style={[styles.recommendationPriorityText, { color: priorityColor }]}>
                    ...
                    <View style={styles.recommendationDetails}>
                      <Text style={styles.recommendationAmount}>
                        Target: {goalsAPI.formatCurrency(recommendation.suggestedAmount)}
                      </Text>
                      <Text style={styles.recommendationTimeframe}>
                        {goalsAPI.formatTimeRemaining(recommendation.suggestedTimeframe * 30)}
                      </Text>
                    </View>
                    ...
                    <TouchableOpacity
                      style={[styles.createFromRecommendationButton, { backgroundColor: typeColor }]}
                      onPress={() => {
                        // Pre-fill create form with recommendation data
                        setShowCreateForm(true);
                      }}
                    >
                      <Text style={styles.createFromRecommendationText}>Create This Goal</Text>
                    </TouchableOpacity>
                  </View>
                );
              })}
