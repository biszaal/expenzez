...
export class BillTrackingAlgorithm {
  ...
  private minConfidence: number = 0.6; // Higher confidence for better accuracy
  private minOccurrences: number = 4; // Require at least 4 occurrences for true recurring bills
  ...
  detectBills(): DetectedBill[] {
    console.log(
      "[BillTrackingAlgorithm] Starting bill detection with",
    ...
    console.log("[BillTrackingAlgorithm] Merchant groups:", {
      ...
      groupSizes: Array.from(merchantGroups.entries()).map(
        ([merchant, txs]) => ({
          merchant,
          count: txs.length,
        })
    ...
    for (const [merchant, transactions] of merchantGroups) {
      ...
      if (transactions.length < this.minOccurrences) {
        console.log(
          "[BillTrackingAlgorithm] Skipping merchant",
          merchant,
          "- only",
          transactions.length,
          "transactions"
      ...
      // Skip if average amount is too small (likely not a real bill)
      const averageAmount =
      ...
      if (averageAmount < 5) {
        console.log(
          "[BillTrackingAlgorithm] Skipping merchant",
          merchant,
          "- average amount too small:",
          averageAmount
        );
        continue;
      }

      // Skip if transactions span less than 2 months (not enough time to establish pattern)
      const sortedTransactions = transactions.sort(
        (a, b) => dayjs(a.date).valueOf() - dayjs(b.date).valueOf()
      );
      const firstTransaction = sortedTransactions[0];
      const lastTransaction = sortedTransactions[sortedTransactions.length - 1];
      const daysBetween = dayjs(lastTransaction.date).diff(
        dayjs(firstTransaction.date),
        "day"
      );
      ...
      if (daysBetween < 60) {
        console.log(
          "[BillTrackingAlgorithm] Skipping merchant",
          merchant,
          "- transactions span too short a period:",
          daysBetween,
          "days"
        );
        continue;
      ...
      console.log(
        "[BillTrackingAlgorithm] Analyzing pattern for",
        merchant,
        "with",
        transactions.length,
        "transactions"
    ...
    console.log(
      "[BillTrackingAlgorithm] Before filtering:",
    ...
    const filteredBills = detectedBills
      .filter((bill) => {
        ...
        const minThreshold =
          MerchantClassifier.getMinConfidenceThreshold(classification);
        ...
        if (!passes) {
          console.log(
            "[BillTrackingAlgorithm] Filtered out bill:",
            bill.merchant,
            "confidence:",
            bill.confidence,
            "threshold:",
    ...
    console.log(
      "[BillTrackingAlgorithm] Final detected bills:",
  ...
  private groupTransactionsByMerchant(): Map<string, Transaction[]> {
    ...
    const typeCount = this.transactions.reduce(
      (acc, tx) => {
    ...
    for (const transaction of this.transactions) {
      ...
      if (
        MerchantClassifier.shouldExcludeFromBills(
          transaction.merchant,
          transaction.description
        )
      ) {
        console.log(
          "[BillTrackingAlgorithm] Excluding retail/variable expense:",
          {
            merchant: transaction.merchant,
            amount: transaction.amount,
            description: transaction.description,
            classification: MerchantClassifier.classifyMerchant(
              transaction.merchant,
              transaction.description
            ),
          }
      ...
      const normalizedMerchant = this.normalizeMerchantName(
        transaction.merchant
      );
  ...
  private normalizeMerchantName(merchant: string): string {
    return merchant
      ...
      .replace(
        /\b(payment|autopay|auto|recurring|subscription|monthly|annual)\b/g,
        ""
      )
  ...
  private groupByAmount(
    ...
    const variance =
      MerchantClassifier.getAmountVarianceThreshold(classification);

    console.log(
      "[BillTrackingAlgorithm] Amount grouping for",
      sampleTransaction.merchant,
      ":",
      {
        classification: classification.type,
  ...
  private detectFrequencyPattern(transactions: Transaction[]): {
    ...
    for (const pattern of patterns) {
      ...
      if (confidence >= 0.4) {
        // At least 40% of intervals match (more lenient)
        const result: any = {
    ...
    if (transactions.length >= 2) {
      ...
      const biWeeklyMatches = intervals.filter(
        (interval) => Math.abs(interval - 14) <= 5
      ).length;
  ...
  private createBillFromPattern(
    ...
    const category = this.categorizeBill(
      merchant,
    ...
    const amountConsistency =
      1 - Math.abs(baseAmount - averageAmount) / baseAmount;
   ...
   */
  private determineBillStatus(
    transactions: Transaction[]
  ): "active" | "cancelled" | "irregular" {
  ...
  private detectSameDayPatterns(): DetectedBill[] {
    ...
    for (const [day, transactions] of dayGroups) {
      if (transactions.length < this.minOccurrences) continue; // Use consistent threshold
      ...
      for (const [merchant, merchantTransactions] of merchantGroups) {
        if (merchantTransactions.length < 3) continue; // Require at least 3 occurrences
        ...
        const amountVariance = amounts.every(
          (amt) => Math.abs(amt - avgAmount) / avgAmount <= 0.15
        );
        ...
        if (amountVariance) {
          ...
          const monthSpan =
            Math.max(...dates.map((d) => d.month())) -
            Math.min(...dates.map((d) => d.month()));
  ...
  static getBillPriority(bill: DetectedBill): number {
    ...
    if (daysUntilDue <= 3)
      priority += 25; // Due very soon
    else if (daysUntilDue <= 7)
      priority += 15; // Due this week
   ...
   */
  static getBillsByCategory(bills: DetectedBill[]): {
    [category: string]: DetectedBill[];
  } {
    return bills.reduce(
      (acc, bill) => {
  ...
  static calculateMonthlyTotal(bills: DetectedBill[]): number {
    return bills.reduce((total, bill) => {
      switch (bill.frequency) {
        case "weekly":
          return total + bill.amount * 4.33; // ~4.33 weeks per month
        case "monthly":
  ...
  static getUpcomingBills(
    ...
    return bills
      .filter(
        (bill) =>
          dayjs(bill.nextDueDate).isBefore(cutoffDate) &&
          bill.status === "active"
      )
      .sort(
        (a, b) =>
