...
const USER_FRIENDLY_MESSAGES: Record<string, string> = {
  ...
  [FrontendErrorCodes.TOKEN_REFRESH_FAILED]:
  ...
  [FrontendErrorCodes.CACHE_ERROR]:
    "Error loading cached data. The app will refresh.",
  [FrontendErrorCodes.STORAGE_ERROR]:
    "Error accessing device storage. Please restart the app.",
  [FrontendErrorCodes.OFFLINE_MODE]:
    "You're currently offline. Some features may be limited.",
  [FrontendErrorCodes.FALLBACK_DATA_USED]:
...
class ErrorHandlerService {
  ...
  transformError(error: any, context?: ErrorContext): ExpenzezFrontendError {
    ...
    const suppressLogging =
      error.suppressErrorLogging === true || error.isOptionalEndpoint === true;
    ...
    if (context?.endpoint?.includes("/auth/")) {
      ...
      // For auth endpoints, preserve the original error structure so AuthContext can detect email verification errors
      ...
      // Preserve original error properties for AuthContext detection
      transformedError.statusCode = error.response?.status || error.statusCode;
      transformedError.response = error.response;
      transformedError.message = errorMessage;
    ...
    if (error.response?.status) {
      ...
      if (status === 429) {
        ...
        return new ExpenzezFrontendError(
          ...
          rateLimitMessage,
          429,
          { originalError: error.message, isAIEndpoint },
          context,
          false,
          true,
  ...
  getUserFriendlyMessage(error: ExpenzezFrontendError): string {
    return (
      ...
      error.message ||
      USER_FRIENDLY_MESSAGES[FrontendErrorCodes.UNKNOWN_ERROR]
    );
  }
  ...
  getRecoveryActions(error: ExpenzezFrontendError): string[] {
    return (
      RECOVERY_ACTIONS[error.code] ||
      RECOVERY_ACTIONS[FrontendErrorCodes.UNKNOWN_ERROR]
    );
  }
  ...
  async handleError(
    ...
    if (transformedError.requiresLogout) {
      console.log(
        `[ErrorHandler] Session expired - using graceful session manager`
      );
      try {
        // Use the new session manager for graceful handling
        const sessionRestored =
          await sessionManager.handleApiSessionExpiration();
        if (sessionRestored) {
          console.log(
            `[ErrorHandler] Session restored successfully - continuing`
          );
          handled = true;
        } else {
          console.log(
            `[ErrorHandler] Session could not be restored - user will be notified`
          );
    ...
    for (const action of recoveryActions) {
      try {
        handled =
          (await this.performRecoveryAction(action, transformedError)) ||
          handled;
      } catch (recoveryError) {
        console.error(
          `Failed to perform recovery action ${action}:`,
          recoveryError
        );
      }
    }
  ...
  private async clearCache(): Promise<boolean> {
    try {
      ...
      if (cacheKeysToRemove.length > 0) {
        ...
        console.log(
          `[ErrorHandler] Cleared ${cacheKeysToRemove.length} cache entries`
        );
      }
  ...
  private logError(error: ExpenzezFrontendError, context?: ErrorContext): void {
    ...
    const isNotificationEndpoint =
      context?.endpoint?.includes("/notifications/");
    const isUsernameCheckEndpoint = context?.endpoint?.includes(
      "/auth/check-user-status"
    );
    const isEmailConfirmEndpoint = context?.endpoint?.includes(
      "/auth/confirm-signup"
    );
    ...
    const isDevelopmentEndpoint =
      ...
      isUsernameCheckEndpoint ||
      isEmailConfirmEndpoint ||
      isLoginEndpoint;
    ...
    if (__DEV__) {
      if (isExpectedBehavior) {
        console.info(
          `ℹ️ [${error.code}] ${error.message} (Expected behavior)`,
          {
            statusCode: error.statusCode,
  ...
  shouldTriggerLogout(error: ExpenzezFrontendError): boolean {
    const logoutTriggers = ["SESSION_EXPIRED", "TOKEN_REFRESH_FAILED"];
